...
export default class ConfirmModel extends Component {
  ...
  render() {
    ...
    const { IsHideCancel, IsHideOk, IsMaxWidth, ChildrenIsComponent, StyleName, cancelBtnTitle, onBtnTitle } = Options || {};
    return (
          ...
          <div className={styles.subLeiLong + ' ' + (!!isShowAction ? styles.showAction : '') + ' '
            + (!!IsMaxWidth ? styles.isMaxWidth : '') + ' ' + styles[StyleName]}>
            {
              ...
              <div className={styles.closeIcon} ref="divClose" >
                <GxIcon ref="divCloseIcon" IconType="iconDelete10" onClick={this.__HandlerCancelButton.bind(this, true)} />
              </div>
            }

            <div className={styles.title}>{Title || '提示'}</div>
